...
private:
  ...
  // Ankush Added variables for implementing CISER
  // The probability of a node when exposed to send a beacon packet. Value between [0, 1]
  Time m_exposedInterval;
  // Check if the node is currently in exposed state. 
  bool m_isExposed;
  // Checks whether this packet is the first packet to be enqueued.
  bool m_isFirstPacket;
  
  double m_exposedMaxJitterMs; // Max Exposed state Jitter in seconds
  
  // Probability of a node becoming a carrier.
  double m_carrierProbability;
  // Probability of the beacons being sent if the node is a carrier.
  double m_carrierRate;
  // Current node is a Carrier or not.
  bool m_isCarrier;
  // Time to reschedule the Carrier checking, and setting.
  Time m_carrierInterval;
  
  double m_carrierMaxJitterMs; // Max Carrier Jitter in seconds


  // Gets called only after a scheduled call. Used to check if the Packet Queue is empty, or if the node can be converted to a carrier node.
  void CarrierCheck();

  // Reset the Exposed state, only called after the exposed state has been invoked.
  void ResetExposedState(); 

  //Check if first packet and set exposed state.
