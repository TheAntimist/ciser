...
TypeId RoutingProtocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::Epidemic::RoutingProtocol")
    ...
    .AddAttribute ("BeaconRandomness","Upper bound of the uniform distribution"
                   ...
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute("ExposedInterval", "The interval time for a node to start sending packets to other nodes.",
                  TimeValue( Seconds(0) ),
                  MakeTimeAccessor (&RoutingProtocol::m_exposedInterval),
                  MakeTimeChecker ())
    .AddAttribute ("ExposedRandomness","Upper bound of the uniform distribution"
                   " random time for Exposed state. Measured in Seconds",
                   DoubleValue (1),
                   MakeDoubleAccessor (&RoutingProtocol::m_exposedMaxJitterMs),
                   MakeDoubleChecker<double> ())
    .AddAttribute("CarrierProbability", "Probability of when a node upon receiving a message becomes a carrier node or not. Should be a value in the range [0, 1]",
                  DoubleValue (0.0),
                  MakeDoubleAccessor (&RoutingProtocol::m_carrierProbability),
                  MakeDoubleChecker<double> (0.0, 1.0))
    .AddAttribute("CarrierRate", "Probability of the packets being sent if the node is a carrier. Should be a value in the range [0, 1]",
                  DoubleValue (1.0),
                  MakeDoubleAccessor (&RoutingProtocol::m_carrierRate),
                  MakeDoubleChecker<double> (0.0, 1.0))
    .AddAttribute ("CarrierInterval","Time to reschedule the Carrier checking, and setting.",
                   TimeValue (Seconds (5)),
                   MakeTimeAccessor (&RoutingProtocol::m_carrierInterval),
                   MakeTimeChecker ())
    .AddAttribute ("CarrierRandomness","Upper bound of the uniform distribution"
                   " random time for Carrier checks. Measured in Seconds",
                   DoubleValue (1),
                   MakeDoubleAccessor (&RoutingProtocol::m_carrierMaxJitterMs),
                   MakeDoubleChecker<double> ())
...
RoutingProtocol::RoutingProtocol ()
  : m_hopCount (0),
    ...
    m_exposedInterval (Seconds(0)),
    m_isExposed(false),
    m_isFirstPacket(true),
    m_exposedMaxJitterMs (0),
    m_carrierProbability (0.0),
    m_carrierRate (0),
    m_isCarrier (false),
    m_carrierInterval( Seconds (0)),
    m_carrierMaxJitterMs (0)
{
...
void
RoutingProtocol::CarrierCheck () {
  NS_LOG_FUNCTION (this);
  if(m_queue.GetSize() == 0)  {
    NS_LOG_DEBUG("Rescheduling CarrierCheck, due to lack of packets.");

    Ptr<UniformRandomVariable> carrierJitter = CreateObject<UniformRandomVariable> ();
    carrierJitter->SetAttribute ("Min", DoubleValue (0.0));
    carrierJitter->SetAttribute ("Max", DoubleValue (m_carrierMaxJitterMs));

    Simulator::Schedule (m_carrierInterval + Seconds(carrierJitter->GetValue()), &RoutingProtocol::CarrierCheck, this);
    return;
  }
  
  Ptr<UniformRandomVariable> testCarrier = CreateObject<UniformRandomVariable> ();
  testCarrier->SetAttribute ("Min", DoubleValue (0.0));
  testCarrier->SetAttribute ("Max", DoubleValue (1.0));
      
  if(testCarrier->GetValue () <= m_carrierProbability) {
    NS_LOG_WARN("Node is in Carrier State.");
    m_isCarrier = true;
  } 
}

void
RoutingProtocol::ResetExposedState () {
  NS_LOG_FUNCTION (this);
  NS_LOG_DEBUG("Resetting from Exposed State. Now in Infected State.");
  // Set a timer for CarrierCheck now that we are in infected state.
  Ptr<UniformRandomVariable> carrierJitter = CreateObject<UniformRandomVariable> ();
  carrierJitter->SetAttribute ("Min", DoubleValue (0.0));
  carrierJitter->SetAttribute ("Max", DoubleValue (m_carrierMaxJitterMs));
...
void
RoutingProtocol::CheckFirstPacket() {
  NS_LOG_FUNCTION(this);
  if(m_isFirstPacket) {
    // NS_LOG_DEBUG("First packet received for node. Going to Exposed State.");
    NS_LOG_WARN("First packet received for node. Going to Exposed State.");
    m_isFirstPacket = false;
    m_isExposed = true;
    
    //Exposed State Check scheduling.

    Ptr<UniformRandomVariable> exposedJitter = CreateObject<UniformRandomVariable> ();
    exposedJitter->SetAttribute ("Min", DoubleValue (0.0));
    exposedJitter->SetAttribute ("Max", DoubleValue (m_exposedMaxJitterMs));

    Simulator::Schedule (m_exposedInterval + Seconds(exposedJitter->GetValue()), &RoutingProtocol::ResetExposedState, this);
  }
}
...
RoutingProtocol::SendBeacons ()
{
  ...
  //   testCarrier->SetAttribute ("Min", DoubleValue (0.0));
  //   testCarrier->SetAttribute ("Max", DoubleValue (1.0));
  //   if(testCarrier->GetValue () <= m_carrierRate) {
  //     NS_LOG_DEBUG("Beacons sent after carrier check.");
  //     BroadcastPacket (packet);
  //   } else
  //     NS_LOG_WARN("Beacons not sent.");
  // } else
  BroadcastPacket (packet);
              ...
              if (tag.GetTagType () == ControlTag::NOT_SET)
                {
                  ...
                  if (m_queue.Find (
                      ...
                      == 0)
                    {
                      NS_LOG_WARN("Packet Delivered locally to " << m_mainAddress << " with ID " << current_Header.GetPacketID ());
  ...
  if (IsMyOwnAddress (header.GetSource ()))
    {
      ...
      NS_LOG_WARN("Packet with ID " << global_packet_ID << " generated at node " << m_mainAddress << " bound for destination " << newEntry.GetIpv4Header().GetDestination());
      
      if(m_isFirstPacket) { //First Created Packet, implies that the node is officially infected.
        m_isFirstPacket = false;
        Ptr<UniformRandomVariable> carrierJitter = CreateObject<UniformRandomVariable> ();
        carrierJitter->SetAttribute ("Min", DoubleValue (0.0));
        carrierJitter->SetAttribute ("Max", DoubleValue (m_carrierMaxJitterMs));
        
        Simulator::Schedule (m_carrierInterval + MilliSeconds(carrierJitter->GetValue()), &RoutingProtocol::CarrierCheck, this); 
      }
    }
  else
    {
      ...
      if (current_Header.GetHopCount () <= 1
              ...
              + m_queueEntryExpireTime) < Simulator::Now () )
        {
          ...
          NS_LOG_WARN("Packet " << current_Header << " reached hop count, thus not added to queue.");
          return true;
        }
      ...
      if (m_queue.Find (current_Header.GetPacketID ()).GetPacketID () == 0)
        NS_LOG_WARN("Packet with ID " << current_Header.GetPacketID() << " generated at another node " << header.GetSource () <<" thus keeping for forwarding.");
      else
        NS_LOG_WARN("Packet with ID " << current_Header.GetPacketID() << " generated at another node " << header.GetSource ()<<" but is a duplicate.");
      CheckFirstPacket(); // Exposed State setting.
    }
  ...
  for (std::vector<uint32_t>::iterator
       ...
       ++i)
    {
      ...
      if (newEntry.GetPacket ())
        {
          if(m_isCarrier && newEntry.GetIpv4Header().GetDestination() != dest) {
            Ptr<UniformRandomVariable> testCarrier = CreateObject<UniformRandomVariable> ();
            testCarrier->SetAttribute ("Min", DoubleValue (0.0));
            testCarrier->SetAttribute ("Max", DoubleValue (1.0));
            if(testCarrier->GetValue () <= m_carrierRate)
              ...
              NS_LOG_WARN("Node in Carrier state." << newEntry.GetPacket() << " Not being sent.");
          } else 
            Simulator::Schedule (Time (0),
                                  &RoutingProtocol::SendPacketFromQueue,
                                 this,dest, newEntry);
...
RoutingProtocol::RecvEpidemic (Ptr<Socket> socket)
{
  // Exposed State 
  if(m_isExposed) {
    NS_LOG_WARN("Node in Exposed State, no summary vectory/packets sent.");
    return;
  }
